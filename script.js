...
function makeCard(id){
  ...
  card.innerHTML = `
    <div class="card-top">
      ...
      <button class="card-icon-btn tree-btn" title="Trace ingredient" aria-label="Trace Ingredient for ${item.name}">🧾</button>
...
function openTreeModal(id){
  const item = DB[id];
  document.getElementById('treeModalTitle').textContent = `${item.emoji} ${item.name} — ingredient trace`;
...
function renderReceipt(){
  ...
  orderedEl.innerHTML = '<p class="r-sec-title">Goods ordered</p>' + orderedIds
    ...
    .map(id => `<div class="r-row ordered" data-id="${id}">
        ...
        <button class="r-minus" title="Reduce quantity" aria-label="Reduce quantity of ${DB[id].name}">−</button>
...
document.getElementById('receiptOrdered').addEventListener('click', e=>{
  const removeBtn = e.target.closest('.r-remove');
  const minusBtn = e.target.closest('.r-minus');
  const btn = removeBtn || minusBtn;
  if(!btn) return;
  const row = btn.closest('.r-row');
  const id = row.dataset.id;
  const itemName = DB[id] ? `${DB[id].emoji} ${DB[id].name}` : 'Item';

  if(minusBtn){
    const willBeZero = (order[id] || 0) <= 1;
    if(willBeZero){
      ...
      row.addEventListener('animationend', ()=>{
        ...
        renderBuildings(document.getElementById('search').value);
        renderReceipt(); renderDashboard();
        toast(`${itemName} removed from order`);
      }, {once:true});
    } else {
      order[id] = order[id] - 1;
      saveData();
